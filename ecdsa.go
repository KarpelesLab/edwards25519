...
func GenerateKeyXY(rand io.Reader) (priv []byte, x, y *big.Int, err error) {
	...
	pub, privArray, err = GenerateKey(rand)
	if err != nil {
...
func SignFromScalar(priv *PrivateKey, nonce []byte, hash []byte) (r, s *big.Int, err error) {
	...
	GeScalarMultBase(&A, privateScalar)
	A.ToBytes(publicKey)
	...
	var localS [32]byte
	ScMulAdd(&localS, &hramDigestReduced, privateScalar,
		nonceLE)
