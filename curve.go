...
func (curve *TwistedEdwardsCurve) recoverXFieldElement(xIsNeg bool, y *FieldElement) *FieldElement {
	// (y^2 - 1)
	l := new(FieldElement)
	FeSquare(l, y)
	FeSub(l, l, &feOne)
	...
	// check (x^2 - x2) % q != 0
	x22 := new(FieldElement)
	FeSquare(x22, x)
	xsub := new(FieldElement)
	FeSub(xsub, x22, x2)
...
func (curve *TwistedEdwardsCurve) IsOnCurve(x *big.Int, y *big.Int) bool {
	...
	yfe := new(FieldElement)
	xfe := new(FieldElement)
	FeFromBytes(yfe, yB)
	FeFromBytes(xfe, xB)
	...
	dx2y2 := new(FieldElement)
	FeMul(dx2y2, &fed, x2)
	FeMul(dx2y2, dx2y2, y2)

	enum := new(FieldElement)
	FeSub(enum, y2, x2)
	FeSub(enum, enum, &feOne)
	FeSub(enum, enum, dx2y2)
...
func toCached(r *cachedGroupElement, p *ExtendedGroupElement) {
...
func (curve *TwistedEdwardsCurve) Add(x1, y1, x2, y2 *big.Int) (x, y *big.Int) {
	...
	aEGE := new(ExtendedGroupElement)
	aEGE.FromBytes(a)

	b := bigIntPointToEncodedBytes(x2, y2)
	bEGE := new(ExtendedGroupElement)
	bEGE.FromBytes(b)
...
func (curve *TwistedEdwardsCurve) ScalarMult(x1, y1 *big.Int, k []byte) (x, y *big.Int) {
	...
	for i := s.BitLen() - 1; i >= 0; i-- {
		dCGE := new(CompletedGroupElement)
		dEGE.Double(dCGE)
